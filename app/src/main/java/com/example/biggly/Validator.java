...
public class Validator {
    ...
    public static  boolean isValidateMultipleEmail(Context context, EditText editText, String message) {
        ...
        if (TextUtils.isEmpty(editText.getText().toString()) ) {
            ...
            String str_newEmail = editText.getText().toString();
            String[] test = str_newEmail.split(",");
            for (int i = 0; i < test.length; i++) {
                if (!test[i].trim().matches(Patterns.EMAIL_ADDRESS.pattern())) {
...
//        textInputLayout.setErrorEnabled(false);
    ...
    public static  boolean isTermsSelected(Context context, CheckBox checkBox, String message) {
